...
public class RedosCheck extends AbstractRegexCheckTrackingMatchType {
  ...
  public void checkRegex(RegexParseResult regexForLiterals, ExpressionTree methodInvocationOrAnnotation, MatchType matchType) {
    ...
    RedosFinder visitor = new RedosFinder(regexForLiterals.getFinalState(), isUsedForFullMatch);
  ...
  private class RedosFinder extends RegexBaseVisitor {
    ...
    private final List<RepetitionTree> repetitions = new ArrayList<>();
    ...
    public RedosFinder(AutomatonState endOfRegex, boolean isUsedForFullMatch) {
    ...
    public void visitRepetition(RepetitionTree tree) {
      boolean canFail = canFail(tree.continuation());
      if (!tree.isPossessive() && tree.getQuantifier().isOpenEnded() && canFail) {
        new BacktrackingFinder(tree.isReluctant(), tree.continuation()).visit(tree.getElement());
      } else {
        super.visitRepetition(tree);
      }
      if (tree.getQuantifier().isOpenEnded() && canFail && canFail(tree)) {
        for (RepetitionTree repetition : repetitions) {
          if (RegexTreeHelper.canReach(repetition, tree)) {
            ...
            if (intersects(auto1, auto2, false) && canFail(tree.continuation())) {
  ...
  private class BacktrackingFinder extends RegexBaseVisitor {
    ...
    public void visitAtomicGroup(AtomicGroupTree tree) {
      new RedosFinder(tree.continuation(), false).visit(tree);
    }
    ...
    boolean containsIntersections(List<? extends AutomatonState> alternatives) {
      for (int i = 0; i < alternatives.size() - 1; i++) {
        ...
        for (int j = i + 1; j < alternatives.size(); j++) {
          ...
          if (intersects(auto1, auto2, false)) {
            return true;
          }
        }
      }
